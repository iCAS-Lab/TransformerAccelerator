...
embedding_partitions="1 2 4 8"
bools="True"

for fc in $fc_out_partition; do
    for emb in $embedding_partitions; do
        for use_conv in $bools; do
            while IFS= read -r model_name
            do
                echo "$model_name $1 $fc $emb $use_conv"
                python3 generate_model_batch.py $model_name $1 $fc $emb $use_conv
